"""
Парсер для Tenisi (tenisi.ru)
API: https://tenisi.ru/api
...
class TenisiParser:
    ...
    def get_sports(self) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return data.get('sports', []) if isinstance(data, dict) else data
        except Exception as e:
            print(f"[Tenisi] Ошибка получения sports: {e}")
        return []
    ...
    def get_events(self, sport_id: Optional[int] = None, live: bool = False) -> List[Dict]:
        ...
        try:
            ...
            print(f"[Tenisi] Ошибка получения events: {e}")
        return []
    ...
    def run(self, interval: int = 10, max_iterations: int = 0):
        ...
        iteration = 0
        print(f"[Tenisi] Запуск парсера (интервал: {interval}с)")
        ...
        try:
            ...
            print("\n[Tenisi] Остановка парсера")
        except Exception as e:
            print(f"[Tenisi] Критическая ошибка: {e}")


if __name__ == "__main__":
    parser = TenisiParser()
    
    print("=== Тест Tenisi Parser ===\n")
